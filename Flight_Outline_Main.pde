...
searchButton airlineSearch;
dialogBox airlineSearchBox;
GroupedData airlineGroup;
...
void setup()
{
  ...
  airlineSearchBox = new dialogBox("Enter an airline code: ");
  airlineSearch = new searchButton(300, 100, 50, 50, searchIcon, airlineSearchBox);
}
void draw()
{
  if (page1) {
    ...
    // this stores the user input as a string that can be passed into the chart functions
    String airlineCode = airlineSearch.draw();
    String[] airlineGroupData = new String[1]; //<>//
    airlineGroupData[0] = airlineCode; //<>//
    airlineGroup = new GroupedData(airlineGroupData, 1, data); //<>//
    Table groupedByAirline = airlineGroup.groupData(); //<>//
    print(groupedByAirline.getRowCount());
    //print("AA: " + groupedByAirline.getInt(1, 3)); // for some reason this says that there is only one row. but in the function itself it works.
